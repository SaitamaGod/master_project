...
#include <stdio.h>
#include <ros/ros.h>
#include "custom_msgs/servo_msg.h"
...
int main(int argc, char **argv){
  ...
  uint16_t dxl1_present_velocity = 0, dxl2_present_velocity = 0;                        // Present velocity
  ...
  ros::Publisher servo_pub = n.advertise<custom_msgs::servo_msg>("servo_topic", 0);
  ros::Rate loop_rate(10);
  custom_msgs::servo_msg msg;
...
#define ESC_ASCII_VALUE                 0x1b
  ...
  while(1)
  {
    ...
    do
    {
      ...
      // Read Dynamixel#1 present velocity
      dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL1_ID, ADDR_MX_PRESENT_VELOCITY, &dxl1_present_velocity, &dxl_error);
      if (dxl_comm_result != COMM_SUCCESS)
      {
        printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
      }
      ...
      msg.id = DXL1_ID;
      msg.pos = dxl1_present_position;
      msg.speed = dxl1_present_velocity;
      ...
      // Read Dynamixel#2 present velocity
      dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL2_ID, ADDR_MX_PRESENT_VELOCITY, &dxl2_present_velocity, &dxl_error);
      if (dxl_comm_result != COMM_SUCCESS)
      {
        printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
      }
      ...
      msg.id = DXL2_ID;
      msg.pos = dxl2_present_position;
      msg.speed = dxl2_present_velocity;
      servo_pub.publish(msg);
      ros::spinOnce();
